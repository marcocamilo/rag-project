...
def preprocessing(
    ...
    if remove_emails:
        cleaned_text = re.sub(
            r"([a-z0-9+._-]+@[a-z0-9+._-]+\.[a-z0-9+_-]+)", " ", cleaned_text
        )
    ...
    if stem or lem or remove_stopwords or tokenize:
        ...
        if remove_stopwords:
            ...
            if lst_stopwords:
                all_stopwords = all_stopwords.union(set(lst_stopwords))
            cleaned_text = [word for word in cleaned_text if word not in all_stopwords]

        # Remove non-existent words
        if exist:
            spell = SpellChecker()
            cleaned_text = [word for word in cleaned_text if spell.known([word])]
